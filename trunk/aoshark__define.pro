...
;+
; aoshark object initialization
;-

function aoshark::Init, root_obj, psf_fname, dark_fname
	...
	self._shark_temp = float(aoget_fits_keyword(fitsheader, 'TEMPI0_S'))
	...
	if exptime eq 0 then begin
		msg_temp = 'Unknown shark exposure time'
		message, msg_temp, /info
        self._shark_err_msg += ' - ' + msg_temp
        valid_exptime = 0B
    endif
...
;stop
    ...
    dark_subdir = ['wfs_calib_'+(root_obj->wfs_status())->wunit(),'shark','backgrounds','bin1']
    if (n_elements(dark_fname) eq 0) then begin
    	if valid_exptime and valid_filt_number then begin
    		...
    		full_dark_fname = self->find_dark(thisJulday, dark_subdir, exptime, self._filter_name, frame_w, frame_h, err_msg=dark_err_msg)
   			if strtrim(dark_err_msg,2) ne '' then self._shark_err_msg += dark_err_msg
   		endif else begin
			msg_temp = 'shark dark cannot be searched: ('
		...
		if not file_test(full_dark_fname) then begin
			msg_temp = 'Overidden shark dark file does not exist'
			message, msg_temp, /info
        	self._shark_err_msg += ' - ' + msg_temp
        endif
	endelse
    ...
    store_radix = filepath(root=root_obj->elabdir(), 'shark')
    ...
    if not self->AOhelp::Init('aoshark', 'shark image') then return, 0
    self->addMethodHelp, "temp()", "shark temperature (K)"
...
;Searches a shark dark closest in time to the shark image with the same set of parameters
;------------------------------------------------------------------------------------------
function aoshark::find_dark, thisJulday, dark_subdir, exptime, filter_tag, frame_w, frame_h, err_msg=err_msg
		...
		if dark_found then begin
			dark_fname = all_darks_fname[idx_closest[dd]]
		 	...
		 	if time_elapsed gt max_time_elapsed then begin
		 		msg_temp = 'Warning: Selected shark dark +'+strtrim(round(time_elapsed/max_time_elapsed),2)+'h old!'
		 		message, msg_temp, /info
		 		err_msg += ' - ' + msg_temp
		 	endif
		endif else begin
			msg_temp = 'No compatible shark dark found (i.e. same exposure time or filter or dimensions)'
			message, msg_temp, /info
			err_msg += ' - ' + msg_temp
			return, ""
		endelse
	endif else begin
		msg_temp = 'No shark darks found matching '+all_darks_search
		message, msg_temp, /info
 		err_msg += ' - ' + msg_temp
		return, ""
	endelse
...
;function aoshark::find_dark_from_tn, thisJulday, dark_subdir, exptime, filter_tag, frame_w, frame_h, ra, dec, stages_x, stages_y, err_msg=err_msg
...
function aoshark::filter_name
	return, self._filter_name
end

function aoshark::temp
	return, self._shark_temp
end

;Returns the error messages
;-----------------------------------------------------
function aoshark::isok, cause=cause
	isok=1B
    isok *= self->AOscientificimage::isok(cause=cause)
	if strtrim(self._shark_err_msg,2) ne '' then begin
		isok*=0B
		cause += self._shark_err_msg
	endif
	return, isok
end
