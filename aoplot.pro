		...
		if ~keyword_set(OVERPLOT) then begin
			if not keyword_set(leg_in) then begin
				...
				if n_elements(H_leg_title) eq 0 then begin
					elab_legend, histo.leg, corners=corners, psym=replicate(8,nvalidbins), linestyle=intarr(nvalidbins), box=box, pspacing=1, CHARSIZE=leg_charsize, clear=clear
				endif else begin
					elab_legend, [[H_leg_title], histo.leg], corners=corners, psym=[[0],replicate(8,nvalidbins)], linestyle=[[-1],intarr(nvalidbins)], box=box, pspacing=1, CHARSIZE=leg_charsize, clear=clear
				endelse
		...
		if ~keyword_set(OVERPLOT) then begin
			if not keyword_set(leg_in) then begin
				if n_elements(g_leg_title) eq 0 then begin
					elab_legend, [g_leg], corners=corners, psym=sym_type[indgen(ng)], linestyle=intarr(ng), box=box, pspacing=1, charsize=leg_charsize, clear=clear
				endif else begin
					elab_legend, [[g_leg_title],g_leg], corners=corners, psym=[[0],sym_type[indgen(ng)]], linestyle=[[-1],intarr(ng)], box=box, pspacing=1, charsize=leg_charsize, clear=clear
				endelse
...
;	 				if not ptr_valid(histo.idxarr[i]) then continue
	...
	if ~keyword_set(OVERPLOT) then begin
		...
		if n_elements(G) ne 0 then begin
			...
			if not keyword_set(leg_in) then begin
				if n_elements(g_leg_title) eq 0 then begin
					elab_legend, g_leg, corners=corners, psym=sym_type[indgen(ng)], linestyle=intarr(ng) $
						 , pos=[0.5*!X.WINDOW[0], !P.REGION[1]], /norm, box=box, pspacing=1, charsize=leg_charsize, clear=clear
				endif else begin
					elab_legend, [[g_leg_title],g_leg], psym=[[0],sym_type[indgen(ng)]], linestyle=[[-1],intarr(ng)] $
						 , pos=[0.5*!X.WINDOW[0], !P.REGION[1]], /norm, box=box, pspacing=1, charsize=leg_charsize, clear=clear
				endelse
			endif else begin
				if n_elements(g_leg_title) eq 0 then begin
					elab_legend, g_leg, psym=sym_type[indgen(ng)], linestyle=intarr(ng), box=box, pspacing=1, charsize=leg_charsize $
		...
		if n_elements(H) ne 0 then begin
			...
			if not keyword_set(leg_in) then begin
				...
				if n_elements(H_leg_title) eq 0 then begin
					elab_legend, histo.leg, psym=replicate(8,nvalidbins), linestyle=intarr(nvalidbins) $
						  , color=histo.cols, pos=h_leg_pos, /norm, box=box, pspacing=1, charsize=leg_charsize, clear=clear
				endif else begin
					elab_legend, [[H_leg_title], histo.leg], psym=[[0],replicate(8,nvalidbins)], linestyle=[[-1],intarr(nvalidbins)] $
