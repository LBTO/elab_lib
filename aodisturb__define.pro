...
function AOdisturb::Init, root_obj, fname, recompute=recompute
    ...
    ;; Special case for atm+sinusmode files generated by the
    ;; Wfs Arbitrator python scripts for gopt during preset.

    ;; Chain header with added disturbance files, if any
    ;; Use sxpar() because the ADD_FILE keyword may span multiple lines
    ;; and aoget_fits_keyword() does not support it.
    add_file = sxpar(header, 'ADD_FILE', count=count)
    if count gt 0 then begin
        p1 = strpos(add_file, 'adsec_calib')
        filename = ao_datadir()+path_sep()+strmid(add_file, p1)
	if (file_info(filename)).exists then begin
            hdr2 = headfits(filename, /silent)
            header = [header, hdr2]

            ; override type
            if strtrim(aoget_fits_keyword(hdr2, 'type'),2) eq 'atm' then begin
                self._type = 'atm+sinus'
            endif
	endif
    endif
